...
keypress(XKeyEvent *ev)
{
	...
	if (ev->state & ControlMask) {
		...
		case XK_w: ksym = XK_Tab;       break;
		case XK_M: ksym = XK_Return; ev->state &= ~ControlMask; break;

		case XK_X: /* delete right */
			text[cursor] = '\0';
			match();
			break;
...
setup(void)
{
	...
	if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
		...
		if (centered) {
			mw = MIN(MAX(max_textw() + promptw, min_width), mw);
...
read_Xresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		if (XrmGetResource(xdb, ".color7", "*", &type, &xval) == True)  /* selected foreground color */
			colors[SchemeSelHighlight][ColFg] = strdup(xval.addr);
		if (dmx < 0 && XrmGetResource(xdb, "dmenu.x", "*", &type, &xval) == True)  /* x position */
			dmx = atoi(xval.addr);
		if (dmy < 0 && XrmGetResource(xdb, "dmenu.y", "*", &type, &xval) == True)  /* y position */
			dmy = atoi(xval.addr);
		if (dmw < 0 && XrmGetResource(xdb, "dmenu.w", "*", &type, &xval) == True)  /* width */
			dmw = atoi(xval.addr);

		XrmDestroyDatabase(xdb);
	}
	colors[SchemeNormHighlight][ColFg] = strdup(colors[SchemeSelHighlight][ColFg]);
